...
app.post('/request-send', (req, res) => {
    ...
    db.query(query, values, (err, result) => {
        if (err) {
            ...
            db.query(supervisorQuery, [idemployees], (err, supervisorResult) => {
                if (err) {
                    console.error('Error fetching supervisor email:', err.stack);
                    res.status(500).send('Error fetching supervisor email');
                    return;
                }

                if (supervisorResult.length === 0) {
                    console.error('Supervisor not found for the employee');
                ...
                const mailOptions = {
                    ...
                    to: supervisorEmail,
                    subject: `แจ้งเตือน${leaveType === 'งานนอกสถานที่' ? 'คำร้องงานนอกสถานที่' : 'คำร้องลา'}จากพนักงาน`,
                    html: `
                        <p>${leaveType === 'งานนอกสถานที่' ? 'คำร้องงานนอกสถานที่' : 'คำร้องลา'}จากพนักงาน:</p>
                        <ul>
                        ...
                        if (err) {
                            ...
                            db.query(updateLeaveQuery, [leaveHours, requestData.idemployees], (err, leaveResult) => {
                                ...
                                const mailOptions = {
                                    ...
                                    html: `
                                        <p>${leaveType === 'งานนอกสถานที่' ? 'คำร้องงานนอกสถานที่' : 'คำร้องลา'}ของคุณ${status === 'อนุมัติแล้ว' ? 'ผ่านการอนุมัติจากหัวหน้าแล้ว' : 'ไม่ผ่านการอนุมัติจากหัวหน้า'}</p>
                                        <ul>
                                            <li>ชื่อพนักงาน: ${employeeName}</li>
